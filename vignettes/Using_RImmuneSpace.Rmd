...
The call to `CreateConnection` instantiates the connection and it initializes itself using the `labkey.*` variables. The `study` parameter can be used to override the `labkey.url.path` string.  Printing the object shows where it's connected, to what study, and the available data sets and gene expression matrices.
