...
ISCon$set(
  ...
  value = function(filterNonGE = TRUE,
    ...
    compDF <- merge(compDF, gea[,-3], by = 0, all = T)
    ...
    DGEA_missing_data <- data.frame(row.names = row.names(gea), missing_timepoints = gea$DGEA_missing)
    ...
    list(mainDF = compDF, DGEA_missing_data = DGEA_missing_data)
  }
)
