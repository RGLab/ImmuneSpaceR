...
# datasets
test_that("get_hai", {
  test_dataset(con, "hai", common_cols, specif_cols = haiCols)
})
test_that("get_elisa", {
  test_dataset(con, "elisa", common_cols, specif_cols = elisaCols)
})
test_that("get_elispot", {
  test_dataset(con, "elispot", common_cols, specif_cols = elispotCols)
})
test_that("get_pcr", {
  test_dataset(con, "pcr", common_cols, specif_cols = pcrCols)
})
test_that("get_gene_expression_files", {
  test_dataset(con, "gene_expression_files", common_cols, specif_cols = gefCols)
})
test_that("fcs_analyzed_result", {
  test_dataset(con, "fcs_analyzed_result", common_cols, specif_cols = farCols)
})
# expression matrices
test_that("get_TIV2008", {
  test_EM(con, "TIV_2008")
})
test_that("get_multiple", {
  test_EM(con, c("TIV_2008", "LAIV_2008"))
})
